 ...
 * @param cwd
 * @param pkg
 * @returns cyanic-eslint-config/index
 * @returns cyanic-eslint-config/react
 * @returns cyanic-eslint-config/typescript/index
 * @returns cyanic-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'cyanic-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
